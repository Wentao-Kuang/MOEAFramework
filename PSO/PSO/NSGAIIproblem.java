...
public class NSGAIIproblem implements Problem {

	public NSGAIIproblem() {
		super();
	...
	public void createExampleData() {
		...
		Frequency = new int[][] { { 103, 25, 30 }, { 13, 5, 24 },
				{ 67, 19, 75 } };
		Latency = new double[][] {
				{ 3.454, 5.556, 0, 2.222, 0, 2.500, 1.434, 1.135, 2.044, 2.444 },
				{ 2.135, 0, 0, 1.55, 2.944, 0.434, 5.135, 0.044, 6.22, 2.33 },
				{ 4.044, 0, 1.15, 2, 1.4, 4.135, 2.144, 0, 2.135, 0.044 } };
	}
	...
	/**
	 * constraint 1, application deployment constraint: each function must be
	 * allocate once
	 */
	...
	public boolean constraint3(boolean[] s) {
		...
		for (int a = 0; a < napplications; a++) {
			for (int v = 0; v < nVMs; v++) {
				if (s[a * nVMs + v]) {
					if ((Acpu[a] <= Computing[v]) && (Aram[a] <= Vram[v])
							&& (Abw[a] <= Vbw[v])) {
						continue;
					} else {
						b = false;
	...
	public static void main(String[] args) {
		NSGAIIproblem c = new NSGAIIproblem();
		c.createExampleData();
