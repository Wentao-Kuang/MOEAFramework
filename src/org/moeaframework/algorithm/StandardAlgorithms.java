 ...
 * algorithms and the customizable properties. These properties are tailored for
 * real-valued operators. If using a different representation, see
 * {@link OperatorFactory} for the appropriate parameters. See the user manual
 ...
 * {@code divisions} by {@code divisionsOuter} and {@code divisionsInner})</td>
 * </tr>
 * <tr>
 * <td>eMOEA</td>
 * <td>Any</td>
 ...
 * pm.distributionIndex, neighborhoodSize, delta, eta, updateUtility}</td>
 * </tr>
 * <tr>
 * <td>MSOPS</td>
 * <td>Real</td>
 * <td>{@code populationSize, numberOfWeights, de.crossoverRate, de.stepSize}</td>
 * </tr>
 * <tr>
 * <td>NSGAII</td>
 * <td>Any</td>
 ...
 * pm.distributionIndex} (for the two-layer approach, replace {@code divisions}
 * by {@code divisionsOuter} and {@code divisionsInner})</td>
 * </tr>
 * <tr>
 * <td>SMPSO</td>
 * <td>Real</td>
 * <td>{@code populationSize, archiveSize, pm.rate, pm.distributionIndex}</td>
 * </tr>
 * <tr>
 * <td>SMS-EMOA</td>
 * <td>Any</td>
 ...
 * <td>DE</td>
 * <td>Real</td>
 * <td>{@code populationSize, method, weights, de.crossoverRate, de.stepSize}</td>
 * </tr>
 * </table>
 * <p>
 ...
 * the result. For example, you can create the algorithm {@code RSO(GA)} to run
...
public class StandardAlgorithms extends AlgorithmProvider {
	...
	public Algorithm getAlgorithm(String name, Properties properties,
		...
		try {
			...
			} else if (name.toUpperCase().startsWith("RSO(")
					&& name.endsWith(")")) {
				typedProperties.setString("algorithm",
						name.substring(4, name.length() - 1));
				return newRSO(typedProperties, problem);
			} else {
				return null;
			}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code eMOEA} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm neweMOEA(TypedProperties properties, Problem problem) {
		...
		final TournamentSelection selection = new TournamentSelection(2,
				comparator);
...
//				tp, problem);
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code NSGAIII}
	 *            instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newNSGAIII(TypedProperties properties, Problem problem) {
		...
		if (properties.contains("divisionsOuter")
				&& properties.contains("divisionsInner")) {
		...
		if (properties.contains("populationSize")) {
			...
			populationSize = (int) (CombinatoricsUtils.binomialCoefficient(
					problem.getNumberOfObjectives() + divisionsOuter - 1,
					divisionsOuter) + (divisionsInner == 0 ? 0
					: CombinatoricsUtils.binomialCoefficient(
							problem.getNumberOfObjectives() + divisionsInner
									- 1, divisionsInner)));
			...
			selection = new Selection() {
				...
				public Solution[] select(int arity, Population population) {
					...
					for (int i = 0; i < arity; i++) {
						result[i] = population.get(PRNG.nextInt(population
								.size()));
					}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code MOEAD} instance
	 * @param problem
	 *            the problem
	 * @return a new {@code MOEAD} instance
	 * @throws FrameworkException
	 *             if the decision variables are not real valued
	 */
	private Algorithm newMOEAD(TypedProperties properties, Problem problem) {
		...
		if (properties.contains("neighborhoodSize")) {
			neighborhoodSize = Math
					.max(2,
							(int) (properties
									.getDouble("neighborhoodSize", 0.1) * populationSize));
		}
		...
		if (properties.contains("eta")) {
			eta = Math.max(2,
					(int) (properties.getDouble("eta", 0.01) * populationSize));
		}

		MOEAD algorithm = new MOEAD(problem, neighborhoodSize, initialization,
				variation, properties.getDouble("delta", 0.9), eta,
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code GDE3} instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code eNSGAII}
	 *            instance
	 * @param problem
	 *            the problem
	...
	private Algorithm neweNSGAII(TypedProperties properties, Problem problem) {
		...
		AdaptiveTimeContinuation algorithm = new AdaptiveTimeContinuation(
				nsgaii, properties.getInt("windowSize", 100), Math.max(
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code CMAES} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newCMAES(TypedProperties properties, Problem problem) {
		...
		int diagonalIterations = (int) properties.getDouble(
				"diagonalIterations", 0);
		String indicator = properties.getString("indicator", "crowding");
		double[] initialSearchPoint = properties.getDoubleArray(
				"initialSearchPoint", null);
		...
		if (problem.getNumberOfObjectives() == 1) {
			...
			archive = new EpsilonBoxDominanceArchive(properties.getDoubleArray(
					"epsilon",
					new double[] { EpsilonHelper.getEpsilon(problem) }));
		}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code PAES} instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code PESA2} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newPESA2(TypedProperties properties, Problem problem) {
		...
		return new PESA2(problem, variation, initialization, bisections,
				archiveSize);
	}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code OMOPSO} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newOMOPSO(TypedProperties properties, Problem problem) {
		...
		int maxIterations = (int) properties.getDouble("maxEvaluations", 25000)
				/ populationSize;
		double mutationProbability = properties.getDouble(
		...
		return new OMOPSO(problem, populationSize, archiveSize, epsilon,
				mutationProbability, perturbationIndex, maxIterations);
	}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code SMPSO} instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code IBEA} instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code SMSEMOA}
	 *            instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code VEGA} instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code DBEA} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newDBEA(TypedProperties properties, Problem problem) {
		...
		int populationSize = (int) (CombinatoricsUtils.binomialCoefficient(
				problem.getNumberOfObjectives() + divisionsOuter - 1,
				divisionsOuter) + (divisionsInner == 0 ? 0 : CombinatoricsUtils
				.binomialCoefficient(problem.getNumberOfObjectives()
						+ divisionsInner - 1, divisionsInner)));
		...
		return new DBEA(problem, initialization, variation, divisionsOuter,
				divisionsInner);
	}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code RVEA} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newRVEA(TypedProperties properties, Problem problem) {
		...
		ReferenceVectorGuidedPopulation population = new ReferenceVectorGuidedPopulation(
				...
				divisionsInner, properties.getDouble("alpha", 2.0));
		...
		int maxGenerations = (int) (properties.getDouble("maxEvaluations",
				10000) / populationSize);
		int adaptFrequency = (int) properties.getDouble("adaptFrequency",
				maxGenerations / 10);
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code RandomSearch}
	 *            instance
	 * @param problem
	 *            the problem
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code MSOPS} instance
	 * @param problem
	 *            the problem
	...
	private Algorithm newMSOPS(TypedProperties properties, Problem problem) {
		...
		List<double[]> weights = new RandomGenerator(
				problem.getNumberOfObjectives(), numberOfWeights).generate();
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new
	 ...
	 */
	private Algorithm newGeneticAlgorithm(TypedProperties properties,
			Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
		double[] weights = properties
				.getDoubleArray("weights", new double[] {});
		...
		if (method.equalsIgnoreCase("linear")) {
			...
			throw new FrameworkException("unrecognized weighting method: "
					+ method);
		}
		...
		return new GeneticAlgorithm(problem, comparator, initialization,
				selection, variation);
	}
	 ...
	 */
	private Algorithm newEvolutionaryStrategy(TypedProperties properties,
			Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		int populationSize = (int) properties.getDouble("populationSize", 100);
		double[] weights = properties.getDoubleArray("weights",
				new double[] { 1.0 });
		String method = properties.getString("method", "linear");
		...
		return new EvolutionStrategy(problem, comparator, initialization,
				variation);
	}
	 ...
	 * 
	 * @param properties
	 *            the properties for customizing the new {@code DE/rand/1/bin}
	 *            instance
	 * @param problem
	 *            the problem
	 * @return a new {@code DE/rand/1/bin} instance
	 */
	private Algorithm newDifferentialEvolution(TypedProperties properties,
			Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		return new DifferentialEvolution(problem, comparator, initialization,
				selection, variation);
	}
